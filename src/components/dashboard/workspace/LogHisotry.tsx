...
const LogHistory = () => {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 text-zinc-150">
      {/* FILTER PANEL SECTION */}
      <section className=" border-b p-4 shrink-0">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-4">
          {/* Text input search */}
          <div className="md:col-span-5 flex items-center  border px-3 p-2 rounded-none focus-within:border-zinc-500 transition-colors">
            ...
            <input
              ...
              className="bg-transparent text-zinc-100 text-xs font-mono w-full outline-none placeholder:text-zinc-700"
              id="history_search_input"
            />
            {searchQuery && (
              <button
                onClick={() => setSearchQuery("")}
                className="text-zinc-500 hover:text-zinc-300 font-bold ml-1.5 text-xs cursor-pointer"
              >
                ×
              </button>
            )}
          ...
          <div className="md:col-span-3">
            <select
              ...
              className="w-full  border text-zinc-300 focus:border-zinc-300 font-mono text-xs p-2.5 rounded-none cursor-pointer outline-none"
          ...
          <div className="md:col-span-4 flex items-center justify-between md:justify-end gap-3 select-none text-xs">
            ...
            {(searchQuery || categoryFilter !== "all" || selectedTagFilter) && (
              <button
                ...
                className=" text-zinc-350 hover:text-zinc-100 border p-2 font-mono text-[10px] uppercase font-bold tracking-wider rounded-none cursor-pointer flex items-center gap-1.5"
              ...
              return (
                <button
                  ...
                  className={`font-mono text-[9px] px-2 py-0.5 border cursor-pointer ${
                    isSelected
                      ? "bg-zinc-100 text-zinc-950 border-zinc-100 font-bold"
                      : " text-zinc-400 hover:border-[#aaa] hover:text-zinc-200"
                  }`}
                >
                  #{tag}
                </button>
          ...
          <div className="flex-1 overflow-y-auto [scrollbar-gutter:stable] pr-1">
            {filteredLogs.length === 0 ? (
              <div className="p-12 border text-center 10">
                ...
                <button
                  onClick={handleClearFilters}
                  className="mt-4 bg-zinc-100 text-zinc-950 p-2 font-mono text-[10px] uppercase font-bold tracking-wider rounded-none cursor-pointer"
                >
                  SHOW_ALL_RECORDS
                </button>
              </div>
                ...
                {filteredLogs.map((log) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Snippet body */}
                      <p className="text-[11.5px] text-zinc-400 line-clamp-3 font-sans leading-relaxed my-2 text-left">
                        {log.whatIDid || (
                          <em className="text-zinc-700">No actions logged</em>
                        )}
                      </p>
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 font-mono text-[11.5px] flex items-center justify-between gap-4 transition-colors duration-75 cursor-pointer ${
                        isSelected
                          ? " text-zinc-100"
                          : "60 hover: text-zinc-400 hover:text-zinc-200"
                      }`}
                    >
                      <div className="flex items-center gap-3 min-w-0 text-left">
                        ...
                        <span className="text-zinc-600 shrink-0 font-bold uppercase">
                      ...
                      <div className="flex items-center gap-3 shrink-0">
                        ...
                        <div className="flex gap-1">
                          {log.tags.slice(0, 1).map((t) => (
                            <span
                              key={t}
                              className="text-[9px] text-zinc-500  px-1 py-0.5 border border-zinc-850"
                            >
                              #{t}
                            </span>
                          ))}
        ...
        <div className="lg:col-span-5 flex flex-col min-h-0 max-h-full">
          {selectedLog ? (
            <div className="border border-zinc-850 rounded-none flex-1 flex flex-col min-h-0 [scrollbar-gutter:stable]">
              {/* Detail Header bar */}
              <div className="p-4 border-b border-zinc-850 flex justify-between items-start gap-4 shrink-0 transition-colors">
                <div className="space-y-1 text-left">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="p-1.5  hover:border border-zinc-850 text-zinc-300 hover:text-zinc-100 rounded-none cursor-pointer transition-colors"
              ...
              <div className="grid grid-cols-3 divide-x divide-zinc-850 border-b border-zinc-850  p-3 shrink-0 text-center font-mono text-xs select-none">
                ...
                <div>
                  ...
                  <span className="text-zinc-200 font-bold block mt-1 uppercase text-[11px]">
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-5 space-y-5 [scrollbar-gutter:stable] /50 text-left">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-1.5 text-zinc-500 font-mono text-[10.5px] font-bold uppercase tracking-wide">
                    <FileText size={12} />
                    <span>01. Operational updates / Tasks did</span>
                  </div>
                  <div className="p-3 border border-zinc-850 rounded-none font-mono text-xs text-zinc-200 leading-relaxed whitespace-pre-wrap select-text">
              ...
              <div className="p-3 border-t border-zinc-850 shrink-0 flex items-center justify-between select-none">
                ...
                <button
                  onClick={handleLoadIntoEditor}
                  className=" text-zinc-205 hover:text-zinc-100 hover:border-zinc-300 border border-zinc-850 py-1.5 px-4 font-mono text-[10px] uppercase font-bold tracking-wider rounded-none cursor-pointer transition-colors"
                >
                  LOAD_INTO_EDITOR
                </button>
              </div>
            ...
            <div className="border border-zinc-850 p-12 text-center 10 rounded-none flex-1 flex flex-col justify-center items-center select-none">
              ...
              <p className="text-[10px] text-zinc-600 mt-2 max-w-xs uppercase">
